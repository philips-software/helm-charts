...
![Version: 0.14.0](https://img.shields.io/badge/Version-0.14.0-informational?style=flat-square) ![AppVersion: 3.8.0](https://img.shields.io/badge/AppVersion-3.8.0-informational?style=flat-square)
...
| features | object | `{"autoInstrumentation":false,"prometheusOperatorObjects":true}` | Feature toggles |
...
| prometheusOperatorObjects | object | `{"serviceMonitors":{"extraMetricProcessingRules":"rule {\n  source_labels = [\"job\"]\n  regex = \"cert-manager/.*\"\n  action = \"drop\"\n}\nrule {\n  source_labels = [\"job\"]\n  regex = \"(.+)/kube-state-metrics\"\n  target_label = \"job\"\n  replacement = \"kube-state-metrics\"\n}\n"}}` | Prometheus Operator Objects configuration |
| prometheusOperatorObjects.serviceMonitors.extraMetricProcessingRules | string | `"rule {\n  source_labels = [\"job\"]\n  regex = \"cert-manager/.*\"\n  action = \"drop\"\n}\nrule {\n  source_labels = [\"job\"]\n  regex = \"(.+)/kube-state-metrics\"\n  target_label = \"job\"\n  replacement = \"kube-state-metrics\"\n}\n"` | Extra metric processing rules for ServiceMonitors (Alloy relabel config syntax). Use this to filter/transform metrics after scraping. Default drops duplicate kube-state-metrics jobs as a workaround for Alloy bug. |
