...
summarize() {
  printf '\n' >&2
  printf '  \033[1;31m🗑️  centcom-satellite uninstall\033[0m \033[2m· about to remove\033[0m\n' >&2
...
done_msg() {
  printf '\n' >&2
  printf '  \033[1;32m✅ centcom-satellite removed from \033[1;36m%s\033[1;32m.\033[0m\n' "$CLUSTER_NAME" >&2
  if [ "$REMOVE_FEDERATION" != true ]; then
    printf '  \033[2m(ClusterFederatedTrustDomain %s kept — set REMOVE_FEDERATION=true to delete)\033[0m\n' "$MCP_FEDERATION_NAME" >&2
  fi
  printf '  \033[2m🤖 Remember to deregister the agent in centcom (id: %s).\033[0m\n\n' "$CLUSTER_NAME" >&2
}
