...
This guide walks you through deploying centcom-satellite to a new Kubernetes cluster and connecting it to centcom.
...
name, Gateway, base domain and cluster name, applies the centcom
...
```bash
curl -fsSL https://raw.githubusercontent.com/philips-software/helm-charts/main/charts/centcom-satellite/install.sh | DRY_RUN=true bash
```

**Override any auto-discovered or baked-in value via env vars** (see the clearly
delimited `BAKED-IN DEFAULTS` block at the top of the script). The centcom
...
```bash
# Onboard a different centcom, custom hostname, pinned chart:
curl -fsSL .../install.sh | \
  MCP_TRUST_DOMAIN=other.example.com \
  HOSTNAME_FQDN=centcom-satellite.edge.example.com \
  CHART_VERSION=0.1.0 bash
```
...
To remove centcom-satellite from the current cluster, use [`uninstall.sh`](uninstall.sh).
...
```bash
curl -fsSL https://raw.githubusercontent.com/philips-software/helm-charts/main/charts/centcom-satellite/uninstall.sh | bash
```
...
- centcom deployed with SPIFFE/SPIRE authentication

## Step 1: Configure SPIRE Federation

The target cluster's SPIRE server must trust the centcom cluster before centcom-satellite can validate incoming requests.
...
metadata:
  name: <centcom-cluster-name>
spec:
  trustDomain: <centcom-trust-domain>
  bundleEndpointURL: <centcom-bundle-endpoint>
...
## Step 2: Create centcom-satellite values file

Create `values-<cluster-name>.yaml`:

```yaml
image:
  tag: v0.49.1  # centcom-satellite version
...
spire:
  ...
  # Trust the centcom cluster
  trustDomains:
    - <centcom-trust-domain>

  # Allow calls from centcom service account
  allowedSPIFFEIDs:
    - spiffe://<centcom-trust-domain>/ns/<centcom-namespace>/sa/<centcom-service-account>

  # JWT audience - must be unique per agent
  jwt:
    enabled: true
    audiences:
      - centcom-satellite-<cluster-name>

# Expose via Gateway API (adjust for your cluster)
httpRoute:
  enabled: true
  hostname: centcom-satellite.<your-domain>
  gatewayRef:
    name: <gateway-name>
...
## Step 3: Deploy centcom-satellite
...
helm upgrade --install centcom-satellite oci://ghcr.io/philips-software/helm-charts/centcom-satellite \
  -n centcom-satellite --create-namespace \
...
```bash
kubectl --context <your-context> get pods -n centcom-satellite -w
```
...
```bash
kubectl --context <your-context> logs -n centcom-satellite deploy/centcom-satellite --tail=20
```
...
## Step 4: Register agent in centcom

Add the new agent to your centcom configuration:

```yaml
agents:
  # ... existing agents ...
  - id: <cluster-name>
    url: https://centcom-satellite.<your-domain>
    jwt_audience: centcom-satellite-<cluster-name>  # Must match Step 2
```

Deploy the updated centcom configuration.
...
```bash
kubectl --context <your-context> get clusterspiffeids centcom-satellite-federated -o yaml
```

Look at `status.stats.entryFailures` - should be 0.

### Check centcom-satellite logs

```bash
kubectl --context <your-context> logs -n centcom-satellite deploy/centcom-satellite -f
```
...
| Empty response from centcom-satellite URL | Expected - requires SPIFFE JWT | Test via centcom, not curl |
| `401 Unauthorized` | JWT audience mismatch | Verify `jwt_audience` in centcom matches `jwt.audiences` in centcom-satellite |
...
| `UPGRADE FAILED: … exists and cannot be imported … missing key "app.kubernetes.io/managed-by"` | A chart resource (e.g. the HTTPRoute) was created outside Helm and lacks Helm ownership metadata | `install.sh` adopts such resources automatically (stamps the Helm label/annotations before upgrading). Disable with `ADOPT_RESOURCES=false`. To fix by hand: `kubectl -n centcom-satellite annotate <kind>/centcom-satellite meta.helm.sh/release-name=centcom-satellite meta.helm.sh/release-namespace=centcom-satellite --overwrite && kubectl -n centcom-satellite label <kind>/centcom-satellite app.kubernetes.io/managed-by=Helm --overwrite` |
...
spec:
  ...
  tls:
  - hosts:
    - centcom-satellite.<your-domain>
    secretName: centcom-satellite-tls
  rules:
  - host: centcom-satellite.<your-domain>
    http:
      paths:
      - path: /
        pathType: Prefix
...
3. Wait for external-dns to create the DNS record (check with `dig centcom-satellite.<your-domain>`)

4. Verify TLS certificate is issued:
```bash
kubectl --context <your-context> get certificate -n centcom-satellite
```
...
By omitting `sectionName` (using empty string), centcom-satellite's HTTPRoute also attaches to all listeners. The Gateway API hostname precedence rules ensure that the specific hostname takes priority over wildcards, so centcom-satellite's route handles the traffic instead of the redirect route.
